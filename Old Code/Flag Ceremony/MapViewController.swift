...
//                        print("anthem not found: \(country.name!)")
    ...
    @objc func showCountry(_ notification: Notification?) {
        ...
        if willGotoNewPosition {
            ...
            mapView!.animate(toPosition: newPosition!,
                             time: 1.0)
        }
    }
    ...
    func showCountryForScreenshot() {
        FirebaseManager.sharedInstance.findCountry(DefaultCountry,
                                                   completion: { (country) in
            if self.hasShownCountryForScreenshot {
                return
            }
            self.hasShownCountryForScreenshot = true
            UserDefaults.standard.set(country!.getGeoRadians()[0],
                                      forKey: kLocationLongitude)
            UserDefaults.standard.set(country!.getGeoRadians()[1],
...
extension MapViewController : MaplyViewControllerDelegate {
    func maplyViewController(_ viewC: MaplyViewController, didSelect selectedObj: NSObject) {
        if let selectedObject = selectedObj as? MaplyScreenLabel {
            ...
            performSegue(withIdentifier: "showCountry",
                         sender: country)
        }
    }
